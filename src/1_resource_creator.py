...
def save_configuration_in_environment(subscription_id: str, resource_group_name: str, aml_workspace_name: str,
    ...
    dotenv.set_key(dotenv_file, 'AZURE_CLI_ML_EXTENSION_ADDED', str(added_extension))
    dotenv.set_key(dotenv_file, 'SUBSCRIPTION_ID', subscription_id)
    dotenv.set_key(dotenv_file, 'RESOURCE_GROUP', resource_group_name)
    dotenv.set_key(dotenv_file, 'AML_WORKSPACE_NAME', aml_workspace_name)
    dotenv.set_key(dotenv_file, 'AML_STORAGE_ACCOUNT', storage_account_name)
    dotenv.set_key(dotenv_file, 'COMPUTE_INSTANCE_NAME', compute_cluster_name)
    dotenv.set_key(dotenv_file, 'ENVIRONMENT_NAME', environment_name)
    dotenv.set_key(dotenv_file, 'AMLW_CLIENT_ID', app_reg_id)
    dotenv.set_key(dotenv_file, 'AMLW_CLIENT_TENANT_ID', app_reg_tenant_id)
    dotenv.set_key(dotenv_file, 'AMLW_CLIENT_APP_ID', app_reg_app_id)
    dotenv.set_key(dotenv_file, 'AMLW_CLIENT_PASSWORD', app_reg_password)
    dotenv.set_key(dotenv_file, 'MODEL_NAME', 'digit-classifier')
